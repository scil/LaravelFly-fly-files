...
class UrlGenerator implements UrlGeneratorContract
{
    ...
    public function __construct(RouteCollection $routes, Request $request)
    {
        ...
        //
        // RouteUrlGenerator has been refactored
    ...
    {
        $dict = static::$corDict[\Swoole\Coroutine::getuid()];
        if ($dict['rootNamespace'] && !(strpos($action, '\\') === 0)) {
            return $dict['rootNamespace'] . '\\' . $action;
        } else {
            return trim($action, '\\');
        }
    }
    ...
    public function formatRoot($scheme, $root = null)
    {
        $dict = &static::$corDict[\Swoole\Coroutine::getuid()];
        if (is_null($root)) {
            if (is_null($dict['cachedRoot'])) {
                $dict['cachedRoot'] = $dict['forcedRoot'] ?: $this->request->root();
            }

            $root = $dict['cachedRoot'];
        }
...
//        }
    ...
    public function setRequest(Request $request)
    {
        ...
        /**
         * hack
         * this prop should not reset any more, because these lines have been moved from method routeUrl
    ...
    {
        static::$corDict[\Swoole\Coroutine::getuid()]['rootNamespace'] = $rootNamespace;
